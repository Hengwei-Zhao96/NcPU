...
def train_CE(args, train_loader, model, loss_fn, optimizer, epoch):
    ...
    for i, (index, images, labels, true_labels) in enumerate(train_loader):
        ...
        # classification loss
        if args.supervised_dataset:
            loss = loss_fn(cls_out, true_labels)
        else:
...
def validate_CE(args, epoch, model, test_loader):
