...
class TrainingDatasetTEDn(Dataset):
...
class TestingDatasetTEDn(Dataset):
...
def get_tedn_dataloader(dataset_name, data_path, positive_class_index, positive_num, unlabeled_num, u_val_num, true_class_prior, batch_size, pos_label):
    ...
    training_p_dataset = TrainingDatasetTEDn(
    ...
    training_p_loader = DataLoader(training_p_dataset, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=2)

    training_u_dataset = TrainingDatasetTEDn(
    ...
    training_u_loader = DataLoader(training_u_dataset, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=2)

    validate_p_dataset = TrainingDatasetTEDn(
    ...
    validate_p_loader = DataLoader(validate_p_dataset, batch_size=batch_size, shuffle=True, num_workers=2)

    validate_u_dataset = TrainingDatasetTEDn(
    ...
    validate_u_loader = DataLoader(validate_u_dataset, batch_size=batch_size, shuffle=True, num_workers=2)

    testing_dataset = TestingDatasetTEDn(images=all_testing_data, true_labels=all_testing_label, transforms=testing_transforms)
