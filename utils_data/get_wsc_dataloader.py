...
class TrainingDatasetnnPU(Dataset):
    def __init__(self, images, labels, true_labels, w_transforms, s_transforms):
        self.images = images
        self.labels = labels
        self.true_labels = true_labels
        self.w_transforms = w_transforms
        self.s_transforms = s_transforms

    def __len__(self):
        return len(self.true_labels)

    def __getitem__(self, index):
        each_image_w = self.w_transforms(self.images[index])
        each_image_s = self.s_transforms(self.images[index])
        each_image_s_ = self.s_transforms(self.images[index])
        each_label = torch.tensor(self.labels[index]).long()

        return each_image_w, each_image_s, each_image_s_, each_label, index
...
class TestingDatasetnnPU(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return each_image, each_true_label, index


def get_wsc_dataloader(dataset_name, data_path, positive_class_index, positive_num, unlabeled_num, true_class_prior, batch_size, pos_label):
    ...
    training_w_transforms, training_s_transforms, testing_transforms = get_transforms(dataset_name=dataset_name)

    training_dataset = TrainingDatasetnnPU(images=training_data, labels=training_labels, true_labels=training_true_labels, w_transforms=training_w_transforms, s_transforms=training_s_transforms)
